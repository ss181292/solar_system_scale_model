...
`poster_models/earth.scad` jest wzorcem dla kolejnych tabliczek. Przy nowej tabliczce kopiuj jego strukturę modułów (`base_plate`, `edge_wedge_x`, `edge_wedge_y`, `corner_wedge`, `frame`, `body_text`, `qr_code`) i podmieniaj tylko dane specyficzne dla ciała niebieskiego (tytuł, treść, macierz QR).
...
- **Ramka o przekroju trójkątnym — NIE używać `difference()`/`hull()` dwóch dużych prostokątów.** Podgląd OpenCSG (domyślny widok bez `--render`) rysuje CAŁY pierwszy operand `difference()` jako wypełnioną bryłę, więc taka ramka wygląda jak "zasłania całą stronę", mimo że finalna geometria po pełnym renderze CGAL byłaby poprawna — to wprowadza w błąd przy podglądzie w GUI. Zamiast tego budować ramkę jako **unię prostych klinów** (`polyhedron()`): `edge_wedge_x`/`edge_wedge_y` dla prostych odcinków (ściana zewnętrzna pionowa, "ostrze" przy wewnętrznej krawędzi) + `corner_wedge` dla narożników (szczyt w rogu zewnętrznym, ostrze w rogu wewnętrznym), połączone samym `union()` bez operacji boolowskich. To renderuje się identycznie w podglądzie i w pełnym renderze.
- **Walidacja `polyhedron()`:** każda krawędź musi występować w dokładnie dwóch ścianach, w przeciwnych kierunkach (konsekwentny "winding"). Błędny winding nie widać w podglądzie OpenCSG, ale powoduje `ERROR: The given mesh is not closed!` przy pełnym renderze i blokuje eksport do STL. Każdy nowy `polyhedron()` waliduj pełnym renderem (`openscad --render -o test.png plik.scad`), nigdy samym podglądem.
